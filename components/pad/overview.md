...
모바일(720px 이하)은 검색줄·도구행과 각 섹션의 큰 글 추가 버튼을 숨겨 캔버스 세로 공간을 확보합니다. 우측 하단의 38~40px 반투명 플로팅 액션이 검색과 글 추가를 담당하고, SECTIONS 레이아웃에서는 가로 스크롤로 가장 가까워진 섹션을 자동 추적해 그 섹션에 글을 씁니다. 다른 레이아웃에 섹션이 여러 개면 작은 섹션 선택 팝오버를 먼저 엽니다. 섹션 생성은 상단 `⋯` 메뉴와 SECTIONS 캔버스 가장 오른쪽의 `새 섹션` 카드를 함께 제공합니다. 모바일에서도 일반 가로 스크롤과 스냅으로 이 카드까지 이동할 수 있으며, 섹션이 하나도 없을 때는 관리자의 첫 화면에 이 카드가 바로 나타납니다.
...
`post-composer.tsx`는 padupgrade.md 6.1~6.3 작성 흐름을 통합합니다. 작성기는 보드가 계속 보이는 우측 패널로 열리고 모바일에서는 `visualViewport`를 따라가는 전체화면으로 전환되어 소프트 키보드 바로 위에 첨부 도구 막대를 유지합니다. 파일·이미지·촬영/녹음·링크·음성·영상·문서 선택과 드래그앤드롭은 같은 검증 큐로 연결됩니다. `usePostDraft`가 제목·본문을 로컬에 자동저장하고 복구·이탈 경고를 제공하지만 상시 자동저장 안내는 표시하지 않습니다. 본문 저장 뒤 `useAttachmentUploadQueue`가 파일별 진행률·취소·재시도와 최대 3개 동시 업로드를 처리합니다. 성공한 파일은 다시 올리지 않고 실패 항목과 실패 링크만 남기므로 게시물 본문을 보존한 채 재시도할 수 있습니다. `LinkPreviewInput`은 여러 URL을 줄 단위로 받아 최대 3개씩 미리보기를 확인하고 중복·전체 첨부 20개 한도를 적용한 다음, 대기 목록의 링크를 게시물 수정 또는 작성 한 번으로 저장합니다. 링크의 검증된 대표 이미지도 이때 함께 저장해 패드 카드와 상세 화면이 제목·출처 호스트가 있는 같은 링크 카드 디자인을 사용합니다.
...
- `pad-more-menu.tsx`는 자주 안 쓰는 보조 기능(보드 활동 기록, 즐겨찾기, 새 섹션, 보관함, 내보내기·발표)을 "⋯" 버튼 하나의 드롭다운으로 묶습니다. `notification-bell.tsx`와 같은 클릭 바깥 감지 패턴을 그대로 씁니다. 항목은 `{key, label, icon, onClick}` 배열로 받으므로 보드 관련 로직은 전혀 모르고, 어떤 항목을 보여줄지는 `pad-canvas.tsx`가 `capabilities`·로그인 여부로 결정합니다.
- 상단바에 항상 보이는 것은 이제 동기화 상태, 다크모드 토글, 알림 벨, 공유 버튼, 더보기(⋯), 설정(관리자만) 6개뿐입니다.
- 720px 이하에서는 보드 도구행 자체를 숨기고, 검색·글 추가를 우측 하단의 축소된 반투명 액션 두 개로 옮깁니다. 모바일 글 추가는 현재 보이는 섹션을 대상으로 하므로 섹션마다 큰 `+` 버튼을 반복하지 않습니다.

보드 설정 모달을 기능별 탭으로 나눈 내용은 `settings/overview.md`를 참고하세요.

## 섹션 "글 추가" 버튼 위치, 보드 소개 압축 (사용자 UX 피드백 반영)

- `section-column.tsx`의 데스크톱 "글 추가" 버튼(`.section-add-trigger`)은 헤더 바로 아래에 유지하고 보드 강조색을 따릅니다. 모바일에서는 같은 버튼이 섹션마다 세로 공간을 반복해서 차지하므로 숨기고, 현재 보이는 섹션을 추적하는 공용 플로팅 글 추가 버튼 하나로 대체합니다.
...
- **섹션 헤더 재압축**: 숫자 인덱스(`01`, `02`…)는 정보 가치가 낮아 제거했습니다. 제목은 헤더 첫 줄에서 한 줄 말줄임으로 표시하고 전체 제목은 기존 편집 진입점의 `title` 속성으로 확인할 수 있어, 모바일에서 각 컬럼의 본문 높이를 더 확보합니다.
...
- **섹션 바로가기 필**(`pad-canvas.tsx`의 `.board-section-pills`): `board.sections`에서 동적으로 만들고 클릭하면 해당 `.section-column`으로 가로 스크롤합니다. 직접 스와이프할 때도 `pad-sections-board.tsx`의 스크롤 프레임 계산이 가장 가까운 섹션을 `activeSectionPill`·`quickSectionId`에 반영해, 모바일 공용 글 추가 버튼이 현재 보이는 섹션을 정확히 대상으로 삼습니다.
...
`pad-canvas.tsx`의 직접 추가 fallback은 일반 로그인 아이디 또는 카카오 이메일을 받고 중립적인 `ch30106` 예시를 사용합니다. 주 설정 UI는 학교 구성원 검색형 `MemberInvitePicker`를 우선 사용합니다.
