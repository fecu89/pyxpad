...
import type { AdminActor, SchoolDirectoryItem, UserRole, UserStatus } from "@/components/admin/types";
...
export function BulkUserActions({ actor, selectedIds, schools, onClose, onApplied }: {
  actor: AdminActor;
  selectedIds: string[];
  ...
  const noFieldSelected = !changeRole && !changeStatus && !changeOrganization;
  ...
  return (
    <form className="admin-bulk-bar" onSubmit={submit}>
      ...
      {!isRepresentative && (
        <>
          <label className="admin-bulk-field">
            <input type="checkbox" checked={changeRole} onChange={(event) => setChangeRole(event.target.checked)} disabled={pending} />
            역할
          ...
          <label className="admin-bulk-field">
            <input type="checkbox" checked={changeStatus} onChange={(event) => setChangeStatus(event.target.checked)} disabled={pending} />
            상태
      ...
      <label className="admin-bulk-field">
        <input type="checkbox" checked={changeOrganization} onChange={(event) => setChangeOrganization(event.target.checked)} disabled={pending} />
        소속
      ...
      <button type="submit" className="button primary" disabled={pending || noFieldSelected}>{pending ? <LoaderCircle size={15} className="spin" /> : null}적용</button>
